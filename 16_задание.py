...
#Алгоритм вычисления значения функции F(n), где n - натуральное число, задан следующими соотношениями:
#F(n) = n, при n 3, при n > 3:
#F(n) = 2*n*n + F(n=1), при чётном n;
#F(n) = n*n*n + n + F(n-1), при нечётном n;
#Определите количество натуральных значений п, при которых F(n) меньше, чем 10**7.
...
print(len([i for i in range(1, len(f)) if f[i] < 10 ** 7]))



#Алгоритм вычисления значения функции F(n), где n - целое число, задан следующими соотношениями:
#F(n) = 1, при n < 2,
#F(n) = F(n/3) - 1, когда n 2 2 и делится на 3,
...
print(len([i for i in range(1, 100001) if F(i)==43]))
...
#Алгоритм вычисления значения функции F(a, b), где а и b - целые неотрицательные числа, задан следующими соотношениями:
#F(a, 0) = a;
#F(a, b) = F(a-1, b) + b, если a 2 b;
#F(a, b) = F(a, b-1) + a, если а < b и b > 0.
#Укажите количество таких целых неотрицательных чисел а, для которых можно подобрать такое b, что F(a, b) = 1 048 576.
