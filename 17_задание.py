...
# Элементы последовательности могут принимать целые значения от 0 до 10 000 включительно. Определите количество пар чисел, в которых хотя бы один из двух элементов больше, чем наибольшее из всех чисел в файле, делящихся на 127,
...
s = [int(x) for x in open('17-243.txt')] #меняеться только название файла
res = [] # будет всегда
max127 = max(t for t in s if t %127==0)
for i in range(len(s)-1):
    if (s[i]>max127 or s[i+1]>max127) and ('31' in oct(s[i])or '31' in oct(s[i+1])):
        res.append(s[i]+s[i+1])
print(len(res), min(res))


# В файле 17-243.txt содержится последовательность целых чисел. Элементы последовательности могут принимать целые значения от 0 до 10 000 включительно.
# Определите количество пар чисел, в которых хотя бы один из двух элементов больше, чем наибольшее из всех чисел в файле, делящихся на 173, и в троичной записи хотя бы одного элемента из двух содержится сочетание цифр 22. 
# В ответе запишите два числа: сначала количество найденных пар, а затем - минимальную сумму элементов таких пар.
# В данной задаче под парой подразумевается два идущих подряд элемента последовательности.
def three(a):
    ss = ''
...
s = [int(x) for x in open('17-243.txt')] #меняеться только название файла
res = [] # будет всегда
max173 = max(t for t in s if t %173==0)
for i in range(len(s)-1):
    if (s[i]>max173 or s[i+1]>max173) and ('22' in three(s[i])or '22' in three(s[i+1])):
        res.append(s[i]+s[i+1])
print(len(res), min(res))


# В файле 17-282.txt содержится последовательность целых чисел. Элементы последовательности могут принимать целые значения от 0 до 10 000.
# Определите количество пар элементов последовательности, в которых хотя бы у одного из чисел сумма цифр троичной записи равна сумме цифр троичной записи максимального элемента последовательности, кратного 11.
# В ответе запишите количество найденных пар, затем минимальную из сумм элементов таких пар.
# В данной задаче под парой подразумевается два идущих подряд элемента последовательности.
def three(a):
    ss = 0
    while a:
        ss +=(a%3)#сумма цифр троичной записи равна сумме цифр троичной записи
        a//=3 #сумма цифр троичной записи равна сумме цифр троичной записи
    return ss #сумма цифр троичной записи равна сумме цифр троичной записи

s = [int(x) for x in open('17-282.txt')] #меняеться только название файла
res = [] # будет всегда
max111 = three(max(t for t in s if t %11==0))
for i in range(len(s)-1):
    if three(s[i])==max111 or three(s[i+1])==max111:
        res.append(s[i]+s[i+1])
print(len(res), min(res))
...
s = [int(x) for x in open('17-354.txt')] #меняеться только название файла
res = [] # будет всегда
min2 = min(t for t in s if abs(t)%10==2)**2
for i in range(len(s)-1):
    if abs((abs(s[i])%10)-(abs(s[i+1])%10))==1 and \
        len([x for x in s[i:i+2]if x %5 ==0])==1 and \
            (s[i]**2+s[i+1]**2)>min2:
        res.append(s[i]+s[i+1])
print(len(res), min(x for x in res if x>0))